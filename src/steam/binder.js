const NovStatsDataBinder = {
  ...
  bindLeetify(data) {
    ...
    if (s.premier_rating != null) {
      document.getElementById('ns-leetify-premier-header').style.display = 'flex';
      this._fillPremierChip('ns-leetify-premier-wrap', s.premier_rating);
    }
  ...
  bindCSStats(data) {
    ...
    if (data.competitive?.length || data.premierSeasons?.length || data.wingman) {
      ...
      panelsEl?.appendChild(this._buildRanksPanels(data));
  ...
  bindFaceit(data) {
    ...
    grid.style.display = 'grid';
    document.getElementById('ns-faceit-profile').style.display = 'flex';
    ...
    if (levelPh && data.level) {
      ...
      img.style.filter  = `drop-shadow(${NovStatsFormatters.faceitLevelColor(data.level)} 0 0 5px)`;
  ...
  _buildRanksPanels(data) {
    ...
    if (data.premierSeasons?.length) {
      const rows = data.premierSeasons.map(s => {
        ...
        return this._makeRankRow(coinSrc, `S${s.season}`, `Season ${s.season}`, s.wins, cur, best);
      });
  ...
  _makeRankSection(title, col1Label, rows, type) {
    ...
    section.className = `ns-ranks-section ns-ranks-section--${type}`;
